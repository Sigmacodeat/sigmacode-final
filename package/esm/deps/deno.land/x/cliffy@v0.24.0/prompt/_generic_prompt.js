...
var _GenericPrompt_value, _GenericPrompt_lastError, _GenericPrompt_isFirstRun, _GenericPrompt_encoder, _GenericPrompt_execute, _GenericPrompt_readKey, _GenericPrompt_readChar, _GenericPrompt_transformValue, _GenericPrompt_validateValue;
import * as dntShim from "../../../../../_dnt.shims.js";
import { tty } from "../ansi/tty.js";
import { parse } from "../keycode/key_code.js";
import { blue, bold, dim, green, italic, red, stripColor } from "./deps.js";
...
export class GenericPrompt {
    constructor(settings) {
        ...
        _GenericPrompt_execute.set(this, async () => {
            ...
            if (typeof GenericPrompt.injectedValue !== "undefined" && __classPrivateFieldGet(this, _GenericPrompt_lastError, "f")) {
        ...
        _GenericPrompt_readChar.set(this, async () => {
            ...
            const isTty = dntShim.Deno.isatty(dntShim.Deno.stdin.rid);
            if (isTty) {
                // cbreak is only supported on deno >= 1.6.0, suppress ts-error.
                dntShim.Deno.setRaw(dntShim.Deno.stdin.rid, true, { cbreak: this.settings.cbreak === true });
            }
            const nread = await dntShim.Deno.stdin.read(buffer);
            if (isTty) {
                dntShim.Deno.setRaw(dntShim.Deno.stdin.rid, false);
            }
            if (nread === null) {
                return buffer;
            }
    ...
    async render() {
        ...
        if (dntShim.Deno.build.os === "windows") {
            ...
            dntShim.Deno.stdout.writeSync(__classPrivateFieldGet(this, _GenericPrompt_encoder, "f").encode(content));
        }
        if (y) {
            this.tty.cursorUp(y);
        }
