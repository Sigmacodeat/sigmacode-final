...
export const loginCmd = new Command()
    .name("login")
    .description(`Log into your upstash account.
...
you can override this with "--config=/path/to/.upstash.json"`)
    ...
    .option("-k, --api-key=<api-key:string>", "Management api apiKey from https://console.upstash.com/account/api")
    .action((options) => {
    const config = loadConfig(options.config);
    if (config) {
        throw new Error(`You are already logged in, please log out first or delete ${options.config}`);
    }
    ...
    if (!email) {
        throw new cliffy.ValidationError(`email is missing, either use "--email" or set "UPSTASH_EMAIL" environment variable`);
    }
    // if (!email) {
    //   if (options.ci) {
    ...
    if (!apiKey) {
        throw new cliffy.ValidationError(`api key is missing, either use "--api-key" or set "UPSTASH_API_KEY" environment variable`);
    }
    // if (!apiKey) {
    //   if (options.ci) {
    ...
    //   );
    // }
    dntShim.Deno.writeTextFileSync(options.config, JSON.stringify({ email, apiKey }));
});
